    ...
    <div id='timeline-content'>
      ...
      <div :style="{width:width+'px'}" style='width:100%;height:100%;position:relative;'>
        ...
        <div id="timeline-cell">
          <Cell v-for='(item,idx) in tbData' :key='idx' :list='item.list'></Cell>
        </div>
...
export default {
  ...
  methods: {
    ...
    combine() {
      items.forEach(v => {
        const _idx = _.findIndex(groups, (o) => { return o.id === v.group })
        if (_idx !== -1) {
          if (!groups[_idx].list) {
            groups[_idx].list = []
          }
